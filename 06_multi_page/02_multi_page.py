...
index_page = html.Div([
    ...
    html.Br(),
    dcc.Link('Podaj swoje doświadccenie', href='/experience')
])
...
experience_layout = html.Div([
    ...
    dcc.RadioItems(
        id='experiance-1-radios',
        options=[{'label': i, 'value': i} for i in ['poniżej 1 roku',
                                                    'od 1 do 3 lat',
                                                    'powyzej 3 lat']],
        value=''
    ),
    html.Div(id='experiance-1-div'),
    html.Br(),
...
@app.callback(
    Output('experiance-1-div', 'children'),
    [Input('experiance-1-radios', 'value')]
)
